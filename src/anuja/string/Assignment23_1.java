...
public class Assignment23_1 {
	...
	void getSpeialChar(String str) {
		...
		for (int index = 0; index < str.length(); index++) {
			char ch = str.charAt(index);
			// write a if condition to compare all above results
			if (!Character.isUpperCase(ch) && !Character.isLowerCase(ch) && !Character.isDigit(ch) && ch != ' ') {
				speicalCharCount++;
